...
#         model="gpt-3.5-turbo",
#         messages=[
#             {"role": "user", "content": f"Translate the following Spanish text into {target_language}: '{text}'. Give just the trsanslation, with no quotes around it."}
#         ],
#         max_tokens=60,
#         top_p=1,
#     )
...
def add_sentence(user_id, include_proper_nouns=False, generateimage=True):
    ...
    try:
        ...
        if len(query.data) == 0:
            sentence_data = {
                ...
                'image_url': generate_image(sentence_text)
            }
        ...
        app.logger.debug("something went right adding card")
        return 1
...
def create_prompt(word_bank):
    return f"{', '.join(word_bank)}\n\n###\n\n" ##this function dec also exists in training.py in model-traning.
